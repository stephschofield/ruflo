...
steps:
  ...
  - action: spawn_headless
    workers:
      ...
      - { role: tester, task: " tests" }
      - { role: docs, task: " documentation" }
...
claude -p " user tests" --session-id test-1 &
wait
...
memory_search  // Find patterns
memory_store   // Store results
