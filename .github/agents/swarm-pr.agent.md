...
agent_spawn { type: "coordinator", name: "PR Coordinator" }
agent_spawn { type: "reviewer", name: "Code Reviewer" }
agent_spawn { type: "tester", name: "Test Engineer" }
agent_spawn { type: "analyst", name: "Impact Analyzer" }
...
const prPreHook = async (prData) => {
  ...
  await swarm_init({ topology, maxAgents: 8 });
  ...
  await coordination_sync({ swarmId: "current" });
};
...
coordination_sync { swarmId: "pr-review-swarm" }
