...
handoffs:
  - agent: coder
    label: Issue requires code implementation
    prompt: issue requires code implementation
  - agent: coordinator
    label: Issue requires broader coordination
    prompt: issue requires broader coordination
user-invocable: true
...
agent_spawn { type: "coordinator", name: "Issue Coordinator" }
agent_spawn { type: "analyst", name: "Issue Analyzer" }
agent_spawn { type: "coder", name: "Solution Developer" }
...
const preHook = async (issue) => {
  ...
  await swarm_init({ topology, maxAgents: 6 });
